...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights, size_t depth=0) {
    ...
    if (depth>4 || !scene_intersect(orig, dir, spheres, point, N, material)) {
        // Calculate background color from the spherical photo envmap
        float u = 0.5 + atan2(dir.x, dir.z) / (2 * M_PI);
        float v = 0.5 - asin(dir.y) / M_PI;
        size_t envmap_x = static_cast<size_t>(u * envmap_width);
        size_t envmap_y = static_cast<size_t>(v * envmap_height);
        Vec3f background_color = envmap[envmap_x + envmap_y * envmap_width];
...
void render(const std::vector<Sphere> &spheres, const std::vector<Light> &lights) {
    ...
    stbi_write_jpg("output.jpg", width, height, 3, pixmap.data(), 100);
}
